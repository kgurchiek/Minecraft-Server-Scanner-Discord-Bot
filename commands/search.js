...
module.exports = {
    ...
    async autocomplete(interaction) {
        ...
        if (focusedValue.name == 'country') await interaction.respond(countryCodes.filter(choice => choice.name.toLowerCase().includes(focusedValue.value.toLowerCase())).splice(0, 25).map(choice => ({
            name: choice.name,
            value: choice.code
        })));
        if (focusedValue.name == 'org') await interaction.respond(orgs.filter(choice => choice.toLowerCase().includes(focusedValue.value.toLowerCase())).splice(0, 25).map(choice => ({
            name: choice,
            value: `^${choice}$`
        })));
    },
    async execute(interaction) {
        // Status message
        const interactReplyMessage = await interaction.reply({
            content: 'Searching...',
            fetchReply: true
        });
        ...
        const searchNextResultCollector = interaction.channel.createMessageComponentCollector({
            filter: searchNextResultFilter
        });
        const searchLastResultCollector = interaction.channel.createMessageComponentCollector({
            filter: searchLastResultFilter
        });
        ...
        function createButtons(totalResults) {
            ...
            searchNextResultCollector.on('collect', async interaction => {
                var newEmbed = new EmbedBuilder()
                    ...
                    .setTimestamp();
                const interactionUpdate = await interaction.update({
                    content: '',
                    embeds: [newEmbed],
                    components: []
                });
                ...
                newEmbed = new EmbedBuilder()
                    ...
                    .addFields({
                        name: 'Result ' + (currentEmbed + 1) + '/' + totalResults,
                        value: '​'
                    }, {
                        name: 'IP',
                        value: server.ip
                        ...
                        name: 'Version',
                        value: getVersion(server.version) + ` (${server.version.protocol})`
                    }, {
                        name: 'Description',
                        value: getDescription(server.description)
                    })
                    .setTimestamp();
                ...
                newEmbed.addFields({
                    ...
                    name: 'Last Seen',
                    value: `<t:${server.lastSeen}:f>`
                })

                await interactionUpdate.edit({
                    content: '',
                    embeds: [newEmbed],
                    components: [buttons]
                });
                ...
                if (location == null) {
                    ...
                    if (location.country != null) {
                        newEmbed.addFields({
                            name: 'Country: ',
                            value: `:flag_${location.country.iso_code.toLowerCase()}: ${location.country.names.en}`
                        })
                    } else {
                        newEmbed.addFields({
                            name: 'Country: ',
                            value: `:flag_${location.registered_country.iso_code.toLowerCase()}: ${location.registered_country.names.en}`
                        })
                    }
                }
                ...
                if (org == null) {
                    ...
                    newEmbed.addFields({
                        name: 'Organization: ',
                        value: org.autonomous_system_organization
                    });
                }
                ...
                if (currentEmbed == oldPage) await interactReplyMessage.edit({
                    content: '',
                    embeds: [newEmbed],
                    components: [buttons]
                });
        ...
        if (isFull.consider) {
            if (isFull.value) {
                mongoFilter['$expr'] = {
                    '$eq': ['$players.online', '$players.max']
                };
            } else {
                mongoFilter['$expr'] = {
                    '$ne': ['$players.online', '$players.max']
                };
            }
            mongoFilter['players'] = {
                '$ne': null
        ...
        if (version.consider) mongoFilter['version.name'] = {
            '$regex': version.value,
            '$options': 'i'
        };
        if (hasImage.consider) mongoFilter['hasFavicon'] = hasImage.value;
        if (description.consider) mongoFilter['$or'] = [{
            'description': {
                '$regex': description.value,
                '$options': 'i'
            }
        }, {
            'description.text': {
        ...
        if (hasPlayerList.consider) {
            ...
            if (hasPlayerList.value) mongoFilter['players.sample']['$not'] = {
                '$size': 0
            };
        }
        if (seenAfter.consider) mongoFilter['lastSeen'] = {
            '$gte': seenAfter.value
        };
        if (ipRange.consider) {
        ...
        if (totalResults > 0) {
            ...
            buttonTimeoutCheck();
            await interactReplyMessage.edit({
                content: '',
                embeds: [newEmbed],
                components: [buttons]
            });
