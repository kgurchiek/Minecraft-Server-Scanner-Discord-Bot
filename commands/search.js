  ...
  async execute(interaction) {
    ...
    function createButtons(totalResults) {
      ...
      searchNextResultCollector.on('collect', async interaction => {
        ...
        server = (await (await fetch(`https://api.cornbread2100.com/servers?limit=1&skip=${currentEmbed}&query=${mongoFilter}`)).json())[0];
    ...
    const totalResults = await (await fetch(`https://api.cornbread2100.com/countServers?query=${mongoFilter}`)).json();

    // If at least one server was found, send the message
    if (totalResults > 0) {
      server = (await (await fetch(`https://api.cornbread2100.com/servers?skip=${currentEmbed}&limit=1&query=${mongoFilter}`)).json())[0];
      hasOldPlayers = server.players.history != null && typeof server.players.history == 'object';
