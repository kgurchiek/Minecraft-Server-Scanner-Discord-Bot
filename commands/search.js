  ...
  async execute(interaction) {
        ...
        if (currentEmbed + 1 < filteredResults.length) {
          ...
          var newEmbed = new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: 'version', value: cleanVersion(filteredResults[currentEmbed].version.name) },
              { name: 'description', value: getDescription(filteredResults[currentEmbed].description) },
              { name: 'players', value: filteredResults[currentEmbed].onlinePlayers + '/' + filteredResults[currentEmbed].maxPlayers }
            )
            .setTimestamp();
    ...
    if (errors.length > 0) {
      ...
      for (var i = 0; i < lastSearchResults.length; i++) {
        ...
        var serverDescription = getDescription(lastSearchResults[i].description);
        ...
        var versionRequirement = cleanVersion(lastSearchResults[i].version.name) == version.value || (cleanVersion(lastSearchResults[i].version.name) + "E").includes(version.value + "E") || version.consider == false;
        var hasImageRequirement = lastSearchResults[i].hasFavicon || hasImage.value == "false" || hasImage.consider == false;
        var descriptionRequirement = (description.consider && description.strict == "false" && serverDescription.includes(description.value)) || (description.consider && description.strict == "true" && serverDescription == description.value) || description.value == "any" || description.consider == false;
      ...
      if (filteredResults.length > 0) {
        ...
        var newEmbed = new EmbedBuilder()
          ...
          .addFields(
            ...
            { name: 'Version', value: cleanVersion(filteredResults[0].version.name) },
            { name: 'Description', value: getDescription(filteredResults[0].description) },
            { name: 'Players', value: filteredResults[0].onlinePlayers + '/' + filteredResults[0].maxPlayers },
            { name: 'Last Seen:', value: `
<t:${filteredResults[0].lastSeen}:f>` }
          )
          .setTimestamp()
