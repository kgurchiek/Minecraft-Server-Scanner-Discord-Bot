...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const totalServers = parseInt(await POST('https://api.cornbread2100.com/countServers', {
            'lastSeen': {
                '$gte': Math.round(new Date().getTime() / 1000) - 3600
            }
        }));
        var index = Math.floor((Math.random() * totalServers));
        const server = (await POST(`https://api.cornbread2100.com/servers?limit=1&skip=${index}`, {
            'lastSeen': {
                '$gte': Math.round(new Date().getTime() / 1000) - 3600
            }
        }))[0];
        ...
        var newEmbed = new EmbedBuilder()
            ...
            .addFields({
                name: `Server ${index}/${totalServers}`,
                value: ' '
            }, {
                name: 'IP',
                value: server.ip
        ...
        await interactionReplyMessage.edit({
            content: '',
            embeds: [newEmbed],
            components: hasOldPlayers ? [buttons] : []
        });
